...
var Bar = function () {
  ...
  _createClass(Bar, [{
    ...
    value: function renderSeries(_ref) {
      ...
      var dataLabels = this.calculateBarDataLabels({ x: x, y: y, i: i, j: j, series: series, realIndex: realIndex, barHeight: barHeight, barWidth: barWidth, renderedPath: renderedPath, visibleSeries: visibleSeries });
      if (dataLabels !== null) {
    ...
    value: function initialPositions(_ref2) {
      var _ref2$makeWidthForVis = _ref2.makeWidthForVisibleItems,
          makeWidthForVisibleItems = _ref2$makeWidthForVis === undefined ? false : _ref2$makeWidthForVis;
    ...
    key: 'drawBarPaths',
    value: function drawBarPaths(_ref3) {
    ...
    key: 'drawColumnPaths',
    value: function drawColumnPaths(_ref4) {
      var indexes = _ref4.indexes,
          x = _ref4.x,
          y = _ref4.y,
          xDivision = _ref4.xDivision,
          pathTo = _ref4.pathTo,
          pathFrom = _ref4.pathFrom,
          barWidth = _ref4.barWidth,
          zeroH = _ref4.zeroH,
          strokeWidth = _ref4.strokeWidth,
          elSeries = _ref4.elSeries;
    ...
    value: function calculateBarDataLabels(_ref5) {
      var x = _ref5.x,
          y = _ref5.y,
          i = _ref5.i,
          j = _ref5.j,
          ...
          visibleSeries = _ref5.visibleSeries,
          renderedPath = _ref5.renderedPath;
    ...
    value: function drawCalculatedBarDataLabels(_ref6) {
      var x = _ref6.x,
          y = _ref6.y,
          val = _ref6.val,
          i = _ref6.i,
          j = _ref6.j,
          dataLabelsConfig = _ref6.dataLabelsConfig;
...
var ApexCharts = function () {
  ...
  _createClass(ApexCharts, [{
      ...
      return new Promise(function (resolve, reject) {
        ...
        if (_this.el !== null) {
          ...
          _this.mount(graphData).then(function () {
            var animations = new _Animations2.default(_this.ctx);
    ...
    value: function create(ser) {
      ...
      var formatters = new _Formatters2.default(this.ctx);
      formatters.setLabelFormatters();
      var titleSubtitle = new _TitleSubtitle2.default(this.ctx);
      titleSubtitle.draw();

      // legend is calculated here before coreCalculations because it affects the plottable area
      new _Legend2.default(this.ctx).init();
    ...
    value: function mount() {
      ...
      return new Promise(function (resolve, reject) {
        ...
        var animations = new _Animations2.default(me.ctx);
        animations.showDelayedElements();
        ...
        if (w.config.xaxis.crosshairs.position === 'front') {
          var crosshairs = new _Crosshairs2.default(me.ctx);
          crosshairs.drawXCrosshairs();
        }

        if (w.config.yaxis[0].crosshairs.position === 'front') {
          var _crosshairs = new _Crosshairs2.default(me.ctx);
        ...
        if (!w.globals.noData) {
          ...
          if (w.globals.axisCharts && w.globals.dataXY) {
            if (w.config.chart.zoom.enabled || w.config.chart.scroller.enabled || w.config.chart.selection.enabled) {
              var zoomPanSelection = new _ZoomPanSelection2.default(me.ctx);
            ...
            if (w.config.chart.scroller.enabled) {
              var scroller = new _Scroller2.default(me.ctx);
          ...
          if (w.config.chart.toolbar.show && !w.globals.allSeriesCollapsed) {
            var toolbar = new _Toolbar2.default(me.ctx);
    ...
    key: 'exec',
    value: function exec(chartID, fn, opts) {
    ...
    value: function draw(series, seriesIndex) {
      ...
      for (var i = 0, bc = 0; i < series.length; i++, bc++) {
        ...
        for (var j = 0, tj = w.globals.dataPoints; j < w.globals.dataPoints; j++, tj--) {
          ...
          elSeries = this.renderSeries({ realIndex: realIndex, pathFill: pathFill, j: j, i: i, pathFrom: pathFrom, pathTo: pathTo, strokeWidth: strokeWidth, elSeries: elSeries, x: x, y: y, series: series, barHeight: barHeight, barWidth: barWidth, elDataLabelsWrap: elDataLabelsWrap, visibleSeries: this.visibleI, type: 'candlestick' });
        }
        ...
        if (w.globals.hasNullValues) {
          if (series[i][j] !== null) {
            if (series[i][j + 1] !== null) {
              ...
              areaPath = graphics.move(pX, pY) + graphics.curve(pX + length, pY, x - length, y, x + 1, y) + graphics.line(x + 1, zeroY) + graphics.line(pX, zeroY) + 'z';
            } else {
...
var Radial = function (_Pie) {
  ...
  _createClass(Radial, [{
    ...
    value: function draw(series) {
      ...
      var size = w.globals.gridWidth / 2.05 - w.config.stroke.width - w.config.chart.dropShadow.blur;
    ...
    value: function drawTracks(opts) {
      ...
      var strokeWidth = opts.size * (100 - parseInt(w.config.plotOptions.radialBar.hollow.size)) / 100 / (opts.series.length + 1) - this.margin;
    ...
    value: function dataLabelsMouseIn(el) {
      ...
      var lbFormatter = w.config.plotOptions.radialBar.dataLabels.value.formatter;
      val = lbFormatter(val);
    ...
    value: function dataLabelsMouseout(el) {
      var w = this.w;
      var dataLabelsGroup = w.globals.dom.baseEl.querySelector('.apexcharts-datalabels-group');
      if (w.config.plotOptions.radialBar.dataLabels.showOn !== 'always' || w.globals.series.length > 1) {
        if (dataLabelsGroup !== null) {
    ...
    value: function renderDataLabels(opts) {
      ...
      if (w.config.plotOptions.radialBar.dataLabels.value.show) {
        var valOffset = w.config.plotOptions.radialBar.dataLabels.name.show ? parseInt(w.config.plotOptions.radialBar.dataLabels.value.offsetY) + 16 : w.config.plotOptions.radialBar.dataLabels.value.offsetY;
...
var Core = function () {
  ...
  _createClass(Core, [{
    ...
    value: function setupElements() {
      ...
      var axisChartsArryTypes = ['line', 'area', 'bar', 'candlestick', 'scatter', 'bubble', 'heatmap'];
  ...
  _createClass(Range, [{
    ...
    value: function handleMinYMaxY() {
        ...
        for (var j = 0; j < gl.series[gl.maxValsInArrayIndex].length; j++) {
          ...
          for (var _i = 0; _i < gl.series.length; _i++) {
            if (gl.series[_i][j] !== null && _Utils2.default.isNumber(gl.series[_i][j])) {
              if (gl.series[_i][j] > 0) {
                poss = poss + parseInt(gl.series[_i][j]);
              } else {
      ...
      cnf.yaxis.map(function (yaxe, index) {
        ...
        if (yaxis[0].min !== undefined && typeof yaxis[0].min === 'number') {
...
var Scroller = function () {
  ...
  _createClass(Scroller, [{
    ...
    value: function renderScroller() {
      ...
      var dim = new _Dimensions2.default(this.ctx);
      var lgRectHeight = dim.getLegendsRect().height - (w.config.legend.position === 'bottom' ? w.config.legend.containerMargin.top : 0) - this.scrollBtnSize / 3;
      var titleHeight = dim.getMainTitleCoords().height;
      var subtitleHeight = dim.getSubTitleCoords().height;

      var scrollerOffsetY = w.config.legend.position === 'top' || w.config.legend.position === 'bottom' ? lgRectHeight + 15 : 15;
    ...
    key: 'renderScrollerTrack',
    value: function renderScrollerTrack() {
      var w = this.w;
      var graphics = new _Graphics2.default(this.ctx);

      var track = graphics.drawRect(0, 0, 0, 0, 4);

      var x = 0;
      var y = -this.scrollerConfig.track.height / 2;
      this.scrollerWidth = w.globals.gridWidth - this.scrollerConfig.padding.left - this.scrollerConfig.padding.right;
      var height = this.scrollerConfig.track.height;
    ...
    key: 'renderScrollerThumb',
    value: function renderScrollerThumb() {
      var w = this.w;
      var graphics = new _Graphics2.default(this.ctx);

      var x = Math.abs(w.globals.initialminX - w.globals.minX) / this.xyRatios.initialXRatio;
      var width = this.scrollerWidth - Math.abs(w.globals.initialmaxX - w.globals.maxX) / this.xyRatios.initialXRatio - x;
      ...
      this.scrollerThumb.draggable(constraint).on('dragmove', this.scrollerDragging.bind(this)).selectize().resize({
    ...
    key: 'styleScrollerButtons',
    value: function styleScrollerButtons() {
      var w = this.w;
      var scrollerButtonLeft = w.globals.dom.baseEl.querySelector('.apexcharts-scroller .svg_select_points_l');
      var scrollerButtonRight = w.globals.dom.baseEl.querySelector('.apexcharts-scroller .svg_select_points_r');
      var scrollerButtons = [scrollerButtonLeft, scrollerButtonRight];
      for (var i = 0; i < scrollerButtons.length; i++) {
        var btn = scrollerButtons[i];
        _Graphics2.default.setAttrs(btn, {
    ...
    key: 'scrollerDragging',
    value: function scrollerDragging() {
      var w = this.w;
      var xyRatios = this.xyRatios;

      var scrollerRect = this.scrollerThumb.bbox();

      var minX = w.globals.initialminX + scrollerRect.x * xyRatios.initialXRatio;
      var maxX = w.globals.initialminX + (scrollerRect.x + scrollerRect.width + this.scrollerConfig.padding.left + this.scrollerConfig.padding.right) * xyRatios.initialXRatio;
...
var ZoomPanSelection = function (_Toolbar) {
  ...
  _createClass(ZoomPanSelection, [{
    ...
    value: function init(_ref) {
      ...
      try {
        for (var _iterator = eventList[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
          ...
          hoverArea.addEventListener(event, self.svgMouseEvents.bind(self, xyRatios), false);
        }
      } catch (err) {
        _didIteratorError = true;
        _iteratorError = err;
    ...
    value: function selectionDrawing(_ref3) {
      ...
      var selectionWidth = me.clientX - gridRectDim.left - startX - 1;
...
var AxesTooltip = function () {
  ...
  _createClass(AxesTooltip, [{
    ...
    value: function drawYaxisTooltipText(index, clientY, seriesBound, xyRatios) {
      var ttCtx = this.ttCtx;
      var w = this.w;
